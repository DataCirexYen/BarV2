...
import {ITimeLockedStakingNFT} from "./interfaces/ITimeLockedStakingNFT.sol";
...
contract SUSHIPOWAH {
    ...
    function balanceOf(address account) external view returns (uint256 powah) {
        ...
        uint256 timeLockedStakingPowah = timeLockedStaking.getUserLockPowah(account); // tally time locked staking powah
        powah = xsushi_powah + lp_powah + toke_powah + timeLockedStakingPowah; // add xSushi & LP weights for 'powah'
    }

    /// @notice Returns total 'powah' supply.
    function totalSupply() external view returns (uint256 total) {
        total = sushi.balanceOf(address(bar)) + sushi.balanceOf(address(pair)) * 2+ sushi.balanceOf(address(timeLockedStaking));
    }
}
